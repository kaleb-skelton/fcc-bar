...
let generateScales = () => {
    ...
    let datesArray = values.map((item) => { return new Date(item[0]) } )  
...
let drawBars = () => {
    ...
    svg.selectAll('rect').data(values).enter().append('rect').attr('class','bar').attr('width', (width - (2 * padding)) / values.length)
